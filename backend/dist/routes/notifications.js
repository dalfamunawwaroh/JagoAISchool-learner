...
router.put('/:id/read', authenticateToken, async (req, res) => {
    ...
    try {
        // Get existing state
        const [existing] = await pool.query('SELECT is_read FROM notifications WHERE id = ? AND user_id = ?', [id, req.userId]);
        if (existing.length === 0) {
            return res.status(404).json({ error: 'Notification not found' });
        }
        const newReadState = !existing[0].is_read;
        await pool.query('UPDATE notifications SET is_read = ? WHERE id = ? AND user_id = ?', [newReadState, id, req.userId]);
...
router.delete('/:id', authenticateToken, async (req, res) => {
    const id = parseInt(req.params.id);
    try {
        const [result] = await pool.query('DELETE FROM notifications WHERE id = ? AND user_id = ?', [id, req.userId]);
