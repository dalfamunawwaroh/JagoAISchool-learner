import fetch from 'node-fetch'; // wait, we can use native fetch or direct pool query.
...
async function main() {
  ...
  try {
    const [result] = await pool.query(
      ...
      WHERE id = ?`,
      [fullName, username, bio || null, websiteUrl || null, linkedinUrl || null, avatarUrl || null, userId]
    );
