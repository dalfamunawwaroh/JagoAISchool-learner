...
router.get('/mentors', async (req, res) => {
  try {
    const [mentors] = await pool.query<any[]>(
      `SELECT m.*, u.full_name, u.avatar_url, u.bio
       FROM mentors m
    ...
    const mentorsWithSkills = await Promise.all(
      mentors.map(async (mentor) => {
        const [skills] = await pool.query<any[]>(
          'SELECT skill_name FROM mentor_skills WHERE mentor_id = ?',
          [mentor.id]
        );
        return {
          id: mentor.id,
...
router.post('/mentorship/sessions', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO mentorship_sessions 
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, 'scheduled')`,
      [req.userId, mentorId, date, timeSlot, topicFocus, blockerDetails || null, zoomLink]
    );
...
router.post('/portfolio/audits', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO portfolio_audits 
       ...
       VALUES (?, ?, ?, ?, 'pending')`,
      [req.userId, portfolioUrl, resumeFileUrl || 'https://jagoaischool.com/uploads/simulated_resume.pdf', targetCareer]
    );
...
router.post('/mock-interviews', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO mock_interviews 
       ...
       VALUES (?, ?, ?, ?, ?, ?, 'scheduled')`,
      [req.userId, careerTrack, formatType, date, time, sandboxLink]
    );
  ...
  try {
    await pool.query(
      `INSERT INTO technical_tickets 
       ...
       VALUES (?, ?, ?, 'open')`,
      [req.userId, urgency || 'low', description]
    );
