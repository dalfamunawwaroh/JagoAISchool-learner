...
router.get('/me', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const [users] = await pool.query<any[]>(
      'SELECT id, email, full_name, username, avatar_url, role, xp, level, language_preference, bio, website_url, linkedin_url, email_notifications FROM users WHERE id = ?',
      [req.userId]
    );
...
router.put('/settings', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    await pool.query(
      ...
      WHERE id = ?`,
      [languagePreference || null, emailNotifications !== undefined ? emailNotifications : null, req.userId]
    );
