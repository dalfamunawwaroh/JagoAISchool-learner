...
router.get('/registered', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const [registrations] = await pool.query(
      `SELECT er.*, e.title, e.event_date, e.event_time, e.image_url
...
router.post('/:id/register', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    // Insert Registration
    await pool.query(
      `INSERT INTO event_registrations (event_id, user_id, full_name, email, whatsapp_number) 
       VALUES (?, ?, ?, ?, ?)`,
      [eventId, req.userId, fullName, email, whatsappNumber]
    );

    const [eventDetails] = await pool.query<any[]>(
      'SELECT title, event_date, event_time FROM events WHERE id = ?',
      [eventId]
    );
...
router.get('/history', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const [history] = await pool.query(
      'SELECT * FROM event_certificates WHERE user_id = ? ORDER BY issued_at DESC',
      [req.userId]
    );
    res.json(history);
  } catch (error) {
