...
router.get('/categories', async (req, res) => {
  try {
    const [categories] = await pool.query(
      `SELECT dc.*, COUNT(dt.id) as count 
...
router.post('/threads', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const [result] = await pool.query<any>(
      'INSERT INTO discussion_threads (title, author_id, category_id) VALUES (?, ?, ?)',
      [title, req.userId, categoryId]
    );
    ...
    if (tags && Array.isArray(tags)) {
      for (const tagName of tags) {
        ...
        // Find or create tag
        let tagId;
        const [existingTags] = await pool.query<any[]>(
          'SELECT id FROM thread_tags WHERE name = ?',
          [cleanTagName]
        );
        ...
        if (existingTags.length > 0) {
          ...
          const [newTagResult] = await pool.query<any>(
            'INSERT INTO thread_tags (name) VALUES (?)',
            [cleanTagName]
          );
...
router.get('/threads/:id/messages', authenticateToken, async (req: AuthenticatedRequest, res) => {
  const threadId = parseInt(req.params.id);

  try {
    const [messages] = await pool.query<any[]>(
      `SELECT cm.*, u.full_name as author_name, u.avatar_url as author_avatar, u.role as author_role,
        ...
        for (const emoji of uniqueEmojis) {
          ...
          if (userIds.length > 0) {
            const [users] = await pool.query<any[]>(
              `SELECT full_name FROM users WHERE id IN (${userIds.join(',')})`
            );
...
router.post('/threads/:id/messages', authenticateToken, async (req: AuthenticatedRequest, res) => {
  const threadId = parseInt(req.params.id);
  ...
  try {
    ...
    await pool.query(
      'UPDATE discussion_threads SET replies_count = replies_count + 1, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
      [threadId]
    );
...
router.post('/threads/:id/like', authenticateToken, async (req: AuthenticatedRequest, res) => {
  const threadId = parseInt(req.params.id);

  try {
    // Check if already liked
    const [existing] = await pool.query<any[]>(
      'SELECT thread_id FROM thread_likes WHERE thread_id = ? AND user_id = ?',
      [threadId, req.userId]
    );
    ...
    if (existing.length > 0) {
      ...
      await pool.query(
        'UPDATE discussion_threads SET likes_count = GREATEST(0, likes_count - 1) WHERE id = ?',
        [threadId]
      );
    ...
    await pool.query(
      'UPDATE discussion_threads SET likes_count = likes_count + 1 WHERE id = ?',
      [threadId]
    );
...
router.post('/messages/:id/react', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const [existingReaction] = await pool.query<any[]>(
