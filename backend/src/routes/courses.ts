...
router.get('/enrollments', authenticateToken, async (req: AuthenticatedRequest, res) => {
  try {
    const [enrollments] = await pool.query(
      `SELECT ce.*, c.title, c.badge, c.level, c.category, c.image_url, c.duration_hours, c.module_count 
...
router.get('/:id', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const [enroll] = await pool.query<any[]>(
...
router.post('/:id/enroll', authenticateToken, async (req: AuthenticatedRequest, res) => {
  const courseId = parseInt(req.params.id);
  ...
  try {
    ...
    await pool.query(
      ...
      [req.userId, courseId, 'active', 0.0]
    );
    ...
    const [courses] = await pool.query<any[]>('SELECT title FROM courses WHERE id = ?', [courseId]);
...
router.post('/lessons/:id/progress', authenticateToken, async (req: AuthenticatedRequest, res) => {
  ...
  try {
    ...
    const [allLessons] = await pool.query<any[]>(
    ...
    const [completedLessons] = await pool.query<any[]>(
      `SELECT lp.lesson_id FROM lesson_progress lp
       JOIN lessons l ON lp.lesson_id = l.id
       JOIN course_modules cm ON l.module_id = cm.id
       WHERE lp.user_id = ? AND cm.course_id = ? AND lp.is_completed = TRUE`,
      [req.userId, courseId]
    );
    ...
    await pool.query(
      ...
      [progressPercentage, status, req.userId, courseId]
    );
